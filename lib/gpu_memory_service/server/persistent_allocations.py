...
class PersistentAllocationManager:
    ...
    def unclaim_all_by_engine(self, engine_id: str) -> int:
        """Release every claim held against ``engine_id``. Used on
        client disconnect when the client doesn't enumerate its
        claims explicitly."""
        exclusive_keys = [k for k in self._exclusive_claimed if k[0] == engine_id]
        shared_keys = [k for k in self._shared_claim_counts if k[0] == engine_id]
        keys = exclusive_keys + shared_keys
        ...
        if keys:
            logger.debug(
                "Unclaimed %d persistent allocations for engine_id=%s",
                len(keys),
                engine_id,
            )
        return len(keys)
