...
path and must keep the shadow-failover e2e green (in the Dynamo container:
`pytest tests/gpu_memory_service/test_shadow_failover.py -k sglang`, with a GMS
server running for the target GPU).
