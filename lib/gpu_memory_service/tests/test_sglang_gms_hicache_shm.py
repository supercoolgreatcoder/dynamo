...
def _config(socket_path, staging_dir):
    return HiCacheStorageConfig(
        ...
        extra_config={
            ...
            "gms_staging_dir": str(staging_dir),
            "gms_staging_rows": 1,
        },
    )


def test_shared_staging_batches_and_survives_engine_restart(tmp_path):
    socket_path = str(tmp_path / "gms.sock")
    daemon, loop, thread = _spawn_daemon(socket_path)
    indices = torch.tensor([0, 1, 2, 3])
    primary_pool = _HostPool([[1, 2, 3, 4], [5, 6, 7, 8]])
    primary = GMSSharedMemoryHiCacheStorage(_config(socket_path, tmp_path))
    primary.register_mem_pool_host(primary_pool)
    try:
        assert primary.batch_set_v1(["native-a", "native-b"], indices) == [True, True]
        assert len(primary._staging_pools) == 1
        primary_paths = [pool.path for pool in primary._staging_pools.values()]
        primary.close()
        assert all(not os.path.exists(path) for path in primary_paths)

        shadow_pool = _HostPool([[0, 0, 0, 0], [0, 0, 0, 0]])
        shadow = GMSSharedMemoryHiCacheStorage(_config(socket_path, tmp_path))
