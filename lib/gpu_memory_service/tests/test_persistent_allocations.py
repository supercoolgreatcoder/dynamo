...
def test_unclaim_all_by_engine(fake_cuda):
    m = PersistentAllocationManager(device=0)
    m.claim("eng-A", "kv_pool", 4096)
    m.claim("eng-A", "weights", 8192)
    m.claim("eng-B", "kv_pool", 4096)
    released = m.unclaim_all_by_engine("eng-A")
    assert released == 2
    assert m.is_claimed("eng-A", "kv_pool") is False
    assert m.is_claimed("eng-A", "weights") is False
    assert m.is_claimed("eng-B", "kv_pool") is True
