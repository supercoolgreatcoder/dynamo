...
# Compare facade-side preprocessing time under the two AGW orchestration paths.
set -euo pipefail
...
"${vc[@]}" get deployment dynamo-pd-preprocessor -o json |
  jq -e --arg binary "$PD_PREPROCESSOR_BINARY" '
    ...
    and any(.spec.template.spec.containers[0].env[];
      .name == "DYN_PREPROCESSOR_STATS_INTERVAL_SECS" and .value == "10")
    and any(.spec.template.spec.containers[0].env[];
      .name == "RUST_LOG" and .value == "warn,dynamo_preprocessor_batch_stats=debug")
  ' >/dev/null
gateway=dynamo-${arm}
...
export RESULT_DIR=$script_dir/results/2026-09-26-mocker-pd-facade-timing
...
if [[ ${PD_CAPTURE_GATEWAY_LOGS:-0} == 1 ]]; then
  "${vc[@]}" logs "deployment/$gateway" > "$RESULT_DIR/gateway-$job.log"
fi
"${vc[@]}" get pods -l app=dynamo-pd-preprocessor -o json |
  jq -r '.items[] | select(.metadata.deletionTimestamp == null) | .metadata.name' |
  while IFS= read -r pod; do
    "${vc[@]}" logs "$pod" > "$RESULT_DIR/preprocessor-$job-$pod.log"
  done
