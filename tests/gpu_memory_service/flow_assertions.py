...
def wait_for_active_layout(
    ...
    *,
    ...
    timeout: float = float(__import__("os").environ.get("GMS_TEST_ACTIVE_LAYOUT_TIMEOUT", "30")),
):
    deadline = time.monotonic() + timeout
    while True:
        weights_state = weights_gms.get_runtime_state()
        kv_state = kv_cache_gms.get_runtime_state()
        logger.warning("[DBG active-layout] weights=%s ro_sess=%s w_alloc=%s w_hash=%s | kv=%s kv_alloc=%s", getattr(weights_state,"state",None), getattr(weights_state,"ro_session_count",None), getattr(weights_state,"allocation_count",None), bool(getattr(weights_state,"memory_layout_hash",None)), getattr(kv_state,"state",None), getattr(kv_state,"allocation_count",None))
        if (
