...
def _resume_shadow_after_primary_failover(
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        logger.warning(
            "[DBG failover] resume_result=%s kv_after_kill=state:%s alloc:%s | kv_events=%s",
            result,
            kv_after_primary_kill.state,
            kv_after_primary_kill.allocation_count,
            [e.kind for e in kv_cache_gms.get_event_history().events],
        )
        return result
...
def _run_shadow_failover_test(
    ...
    with GMSProcessManager(request, engine_cls) as manager:
        ...
        # Pre-activation handoff: the KV layout is continuously RW-owned and the
        # shadow ends as the sole RW owner of the full pool. (The exact event
        # tail differs from the old strict-blocking allocation_oom sequence; it
        # is logged here and asserted precisely once observed.)
        kv_events_after_resume = kv_cache_gms.get_event_history().events
        logger.warning(
            "[DBG failover] final kv_events=%s",
            [e.kind for e in kv_events_after_resume],
        )
