...
class BaseWorkerHandler(ABC, Generic[RequestT, ResponseT]):
    ...
    def __init__(
        ...
        # Serializes encode() across concurrent requests: the encoder instance is
        # process-global and runs via asyncio.to_thread(), so without this two
        # requests could call user encoder code concurrently and race on shared
        # model/device state.
        self._custom_encoder_lock = asyncio.Lock()
...
class DecodeWorkerHandler(BaseWorkerHandler):
    ...
    async def _assemble_custom_encoder_prompt(
        ...
        try:
            # Serialize the user encoder's forward pass on the shared instance.
            async with self._custom_encoder_lock:
                img_tensors: list[torch.Tensor] = await asyncio.to_thread(
                    self._custom_encoder.encode, image_urls
                )
