...
class RankLivenessClient:
    ...
    def __init__(self, leader_host: str, rank: int, *, interval_ms: Optional[int] = None):
    ...
    def start(self) -> None:
        ...
        logger.info(
            ...
            self._rank,
            leader_connect_addr(self._leader_host),
    ...
    def _run(self) -> None:
        ...
        sock.connect(leader_connect_addr(self._leader_host))
        try:
...
class RankLivenessMonitor:
    """Runs on the leader (rank 0, the active engine that holds the failover lock).
    Binds a ROUTER, tracks the last heartbeat per worker rank, and calls
    ``on_rank_lost(rank, reason)`` exactly once when a *previously seen* rank goes
    silent longer than the timeout. Only ranks that have connected are armed, so a
    worker that is still starting up never produces a false loss."""
    ...
    def __init__(
        ...
        *,
        ...
        self._timeout = (timeout_ms_override or timeout_ms()) / 1000.0
        self._stop = threading.Event()
        self._thread: Optional[threading.Thread] = None
        self._fired = False
    ...
    def _run(self) -> None:
        ...
        try:
            while not self._stop.is_set():
                ...
                if sock in events:
                    try:
                        identity = sock.recv(zmq.NOBLOCK)
                        _ = sock.recv(zmq.NOBLOCK)  # payload (b"hb"), ignored
                        rank = self._rank_of(identity)
                        if rank is not None:
                        ...
                        pass
                # Arm only ranks that have connected; fire once on the first loss.
                for rank, seen in list(last_seen.items()):
                    if now - seen > self._timeout and not self._fired:
                        self._fired = True
                        logger.warning(
                            "[GMS liveness] rank %d silent for %.0fms (>%.0fms) — "
                            "treating as dead, triggering failover",
                            rank,
                            (now - seen) * 1000,
                            self._timeout * 1000,
                        )
                        try:
                            self._on_rank_lost(rank, "liveness-timeout")
                        except Exception:
